...
namespace HousingHubBackend.Controllers
{
    ...
    public class UsersController : ControllerBase
    {
        ...
        public IActionResult HashAllPasswords()
        {
            ...
            {
                // Only hash if not already hashed (simple check for Identity hash prefix)
                if (!string.IsNullOrEmpty(user.Password) && !user.Password.StartsWith("$2") && !user.Password.StartsWith("AQAAAA"))
                {
            ...
            _context.SaveChanges();
            return Ok(new { message = $"{updated} user passwords hashed." });
        }
    }
}