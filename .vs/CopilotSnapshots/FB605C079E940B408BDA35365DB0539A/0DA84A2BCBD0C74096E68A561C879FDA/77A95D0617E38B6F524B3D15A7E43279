...
const AnnouncementsPage = () => {
    ...
    const fetchAnnouncements = async () => {
        try {
            ...
            // Resident: show announcements for their flat, wing, or whole society
            if (user.role === 'resident') {
                setAnnouncements(data.filter(a => {
                    ...
                    // Wing check: if any of the announcement's targetWingIds matches the resident's wingId
                    if (Array.isArray(a.targetWingIds) && user.flatId) {
                        const myFlat = flats.find(f => f.flatId === user.flatId);
                        // Try both f.wingId and f.wing.wingId for robustness
                        const residentWingId = myFlat?.wingId || myFlat?.wing?.wingId;
                        if (residentWingId && a.targetWingIds.map(Number).includes(Number(residentWingId))) return true;
                    }
                    return false;
                }));
            } else {
            ...
            <div className="bg-white rounded-lg shadow p-6">
                {user.role === 'admin' && (
                    <form onSubmit={handleSubmit} className="flex flex-wrap gap-2 mb-4 items-end">
                        ...
                        <div className="flex flex-col">
                            <label htmlFor="content" className="font-semibold mb-1">Message</label>
