...
const FlatsList = () => {
  ...
  const [form, setForm] = useState({ flatNumber: '', floorNumber: '', area: '', status: 'Vacant', wingId: '' });
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      setForm({ flatNumber: '', floorNumber: '', area: '', status: 'Vacant', wingId: '' });
      setEditingId(null);
      fetchFlats();
    } catch {
  ...
  return (
    <div>
      ...
      <form onSubmit={handleSubmit} className="flex flex-wrap gap-2 mb-4 items-end">
        ...
        <Input name="floorNumber" value={form.floorNumber} onChange={handleChange} placeholder="Floor Number" type="text" required />
        <Input name="area" value={form.area} onChange={handleChange} placeholder="Area (sq ft)" type="number" min="0" step="0.01" required />
        ...
        {editingId && (
          <Button type="button" variant="secondary" onClick={() => { setForm({ flatNumber: '', floorNumber: '', area: '', status: 'Vacant', wingId: '' }); setEditingId(null); }}>
            Cancel
          </Button>
        )}
      </form>
